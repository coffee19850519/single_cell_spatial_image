...
def transform_embedding_to_image(embedding_file, meta_data_file, save_folder,scale_factor_file= None,
    ...
    save_transformed_RGB_to_image_and_csv(full_data['pxl_col_in_fullres'].values,
                                          ...
                                          os.path.join(save_folder, os.path.basename(embedding_file)),
...
if __name__ == "__main__":
    data_fold = r'/run/media/fei/Entertainment/scGNN_4_embedding/'
    save_folder = r'/run/media/fei/Entertainment/scGNN_4_embedding/velocity_images_distance/'
    norm_list = ['sctransform', 'cpm']
    all_data_names = []
    ...
    for data_name in all_data_names:
        ...
        for dirs in glob(os.path.join( data_fold, 'Velocity Based on Distance',
                                       'outputdirS_' + data_name + '_*')):
                             # 'outputdirS_'+ data_name + '_' + norm + '_*')):
                if 'NORESULT' in dirs:
                    continue
                for file_name in os.listdir(dirs):
                    if os.path.splitext(file_name)[1] != '.csv' or \
                        os.path.basename(file_name).rsplit('_', 1)[1] == '0.csv':
                        continue
                    embedding_file = os.path.join(dirs, file_name)
                    ...
                    try:
                        transform_embedding_to_image(embedding_file = embedding_file,
