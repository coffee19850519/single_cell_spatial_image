...
def generateAdj(featureMatrix, graphType='KNNgraph', para = None, parallelLimit = 0, adjTag = True, spatialMatrix = None ):
    """
    Generating edgeList 
    """
    edgeList = None
    ...
    if graphType == 'KNNgraphPairwise':
        ...
        # Spatial Lattice; spatial Grid X,Y as the edge, only the exact edge as the grid
        if para != None:
            parawords = para.split(':')
            distanceType = parawords[0]
            k = int(parawords[1])
...
def generateAdjWeighted(featureMatrix, graphType='KNNgraph', para = None, parallelLimit = 0, adjTag = True, spatialMatrix = None ):
    """
    ...
    if graphType == 'KNNgraphStats':
        ...
        if para != None:
            ...
            distanceType = parawords[0]
            k = int(parawords[1])
        edgeListWeighted = calculateSpatialMatrixWeighted(featureMatrix, distanceType=distanceType, k=k, spatialMatrix=spatialMatrix)
    else:
        print('Should give graphtype')
...
def generateMVedgeList(eList1, eList2, nodeNum, MVop='intersect'):
    if MVop == 'intersect':
        edgeList = intersection(eList1,eList2)
    elif MVop == 'union':
        edgeList = union(eList1,eList2)
    elif MVop == 'concat':
        edgeList = concat(eList1,eList2)
    graphdict = edgeList2edgeDict(edgeList, nodeNum)
    adj = nx.adjacency_matrix(nx.from_dict_of_lists(graphdict))
    return adj, edgeList
...
def calculateSpatialMatrix(featureMatrix, distanceType='euclidean', k=6, pruneTag='NA', spatialMatrix=None):
    ...
    for i in np.arange(spatialMatrix.shape[0]):
        ...
        for j in np.arange(1,k+1):
            ...
            # Prune: check, only exclude large outliners wihtin one STD
            # if (distMat[0,res[0][j]]<=mean+std) and (distMat[0,res[0][j]]>=mean-std):
                if distMat[0,res[0][j]]<=boundary:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            # Prune: only use nearest neighbor as exact grid: 6 in cityblock, 8 in euclidean
                ...
                if distMat[0,res[0][j]]<=2.0:
                    ...
                    edgeList.append((i,res[0][j],0.0))
            # Prune: only use nearest/second nearest neighbor as exact grid: 12 in cityblock, 16 in eculidean
            elif pruneTag == 'GridEx':
                if distMat[0,res[0][j]]<=4.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            # Prune: only use nearest/second nearest neighbor as exact grid: 18 in cityblock, 24 in eculidean
            elif pruneTag == 'GridEx2':
                if distMat[0,res[0][j]]<=6.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx3':
                if distMat[0,res[0][j]]<=8.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx4':
                if distMat[0,res[0][j]]<=10.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx5':
                if distMat[0,res[0][j]]<=12.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx6':
                if distMat[0,res[0][j]]<=14.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx7':
                if distMat[0,res[0][j]]<=16.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx8':
                if distMat[0,res[0][j]]<=18.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx9':
                if distMat[0,res[0][j]]<=20.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx10':
                if distMat[0,res[0][j]]<=22.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx11':
                if distMat[0,res[0][j]]<=24.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx12':
                if distMat[0,res[0][j]]<=26.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx13':
                if distMat[0,res[0][j]]<=28.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx14':
                if distMat[0,res[0][j]]<=30.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx15':
                if distMat[0,res[0][j]]<=32.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx16':
                if distMat[0,res[0][j]]<=34.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx17':
                if distMat[0,res[0][j]]<=36.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx18':
                if distMat[0,res[0][j]]<=38.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx19':
                if distMat[0,res[0][j]]<=40.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx24':
                if distMat[0,res[0][j]]<=50.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
            elif pruneTag == 'GridEx29':
                if distMat[0,res[0][j]]<=60.0:
                    edgeList.append((i,res[0][j],1.0))
                else:
                    edgeList.append((i,res[0][j],0.0))
...
def calculateSpatialMatrixWeighted(featureMatrix, distanceType='euclidean', k=6, spatialMatrix=None):
    r"""
    Calculate spatial Matrix directly use X/Y coordinates weighted
    """       
    edgeListWeighted=[]
    ...
    for i in np.arange(spatialMatrix.shape[0]):
        ...
        for j in np.arange(1,k+1):   
            ...
            if weight<0.01:
                edgeListWeighted.append((0,res[0][j],0.0))
            else:
                edgeListWeighted.append((0,res[0][j],weight))
    
    return edgeListWeighted

# kernelDistance
def kernelDistance(distance,delta=0.5):
    '''
