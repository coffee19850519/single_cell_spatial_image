...
# Original Version: 
#gae embedding: adjSource equals to adjTarget
...
def preprocessGraph(adj):
    ...
    adj_train, train_edges, val_edges, val_edges_false, test_edges, test_edges_false = mask_test_edges(adj)
    return adj_train, adj_orig, test_edges, test_edges_false

# Updated Version: 
# gae embedding MultiView: adjSource may be different from adjTarget
def GAEembeddingMultiView(z, adjSource, adjTarget, args):
    '''
    GAE embedding for clustering
    Param:
    ...
    adjSource, _, _, _ = preprocessGraph(adjSource)
    adjTarget, adj_orig, test_edges, test_edges_false = preprocessGraph(adjTarget)
    adj_train = adjTarget   

    # Some preprocessing
    adj_norm = preprocess_graph(adjSource)
    adj_label = adj_train + sp.eye(adj_train.shape[0])
    # adj_label = torch.DoubleTensor(adj_label.toarray())
    adj_label = torch.FloatTensor(adj_label.toarray())

    pos_weight = float(adjTarget.shape[0] * adjTarget.shape[0] - adjTarget.sum()) / adjTarget.sum()
    norm = adjTarget.shape[0] * adjTarget.shape[0] / float((adjTarget.shape[0] * adjTarget.shape[0] - adjTarget.sum()) * 2)
