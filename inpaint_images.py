...
# def save_spot_RGB_to_image(data_folder, expression_file, scale_factor_file):
#     data_file = os.path.join(data_folder, expression_file)
#     X = pd.read_csv(data_file)
#     tissue = X.iloc[:, 1].values
#     # layers = X.iloc[:, 6].values
#     #
...
#
#     radius = round(radius * (600/max_row))
#     # print(radius)
#
#     spot_row = X.iloc[:, 4].values * (600/max_row)
#     spot_row = spot_row.astype(np.int)
#     spot_col = X.iloc[:, 5].values * (600/max_col)
...
def inpaint(img_path, anndata, metadata_all):
    for name in os.listdir(img_path):
        ...
        for i in range(len(out_tissue)):
            # print(out_tissue[i])
            dist = cv2.pointPolygonTest(contours[max_idx], (out_tissue[i][1],out_tissue[i][0]),False)
            if dist == 1.0:
                ...
                inpaint_path = img_path.split('/')[0]+'/'+sample+"/pseudo_image_inpaint/"
