import pandas as pd
from pipeline_sparse_expression_to_image import save_transformed_RGB_to_image_and_csv, scale_to_RGB,get_boundary
import json, os
import numpy as np
...
def transform_embedding_to_image(anndata,sample_name,img_folder, img_type, scale_factor_file= None):
    ...
    for i in range(3):
        minimum,maximum,maxplus = get_boundary(X_transform[:, i])
        X_transform[:, i] = X_transform[:, i] * (5 - (-5)) / (maximum - minimum)

    X_transform[:, 0] = torch.sigmoid(torch.tensor(X_transform[:, 0]))
    X_transform[:, 1] = torch.sigmoid(torch.tensor(X_transform[:, 1]))
    X_transform[:, 2] = torch.sigmoid(torch.tensor(X_transform[:, 2]))
