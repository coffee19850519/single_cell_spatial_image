...
def loss_function_graph(recon_x, x, mu, logvar, graphregu=None, gammaPara=1.0, regulationMatrix=None, regularizer_type='noregu', reguPara=0.001, modelusage='AE', reduction='sum'):
    ...
    BCE = vallina_mse_loss_function(recon_x, target, reduction=reduction)
    ...
    # https://arxiv.org/abs/1312.6114
    # 0.5 * sum(1 + log(sigma^2) - mu^2 - sigma^2)
    if modelusage == 'VAE' or modelusage == 'PVAE':
        KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        loss = loss + KLD
...
def loss_function_graph_celltype(recon_x, x, mu, logvar, graphregu=None, celltyperegu=None, gammaPara=1.0, regulationMatrix=None, regularizer_type='noregu', reguPara=0.001, reguParaCelltype=0.001, modelusage='AE', reduction='sum'):
    ...
    if regularizer_type == 'noregu':
        ...
        loss = BCE + reguPara * graph_mse_loss_function(recon_x, target, graphregu=graphregu, reduction=reduction) + \
            reguParaCelltype * graph_mse_loss_function(recon_x, target, graphregu=celltyperegu, reduction=reduction)
...
def loss_function_position(z_mu, z_logvar, y, y_recon, beta, reduction='mean'):
    # reconstruction error
    B=y.shape[0]
    gen_loss = F.mse_loss(y_recon, y.view(B, -1))
    # gen_loss = (y - y_recon) ** 2
    # if reduction != 'none':
    #     gen_loss = torch.mean(gen_loss) if reduction == 'mean' else torch.sum(gen_loss)
    # latent loss
    kld = torch.mean(-0.5 * torch.sum(1 + z_logvar -
                                      z_mu.pow(2) - z_logvar.exp(), dim=1), dim=0)
    # total loss
    loss = gen_loss + beta*kld/y_recon.shape[1]
...
def loss_function_position_AE(y, y_recon, beta, reduction='mean'):
    # reconstruction error
    B=y.shape[0]
    loss = F.mse_loss(y_recon, y.view(B, -1))
    return loss
...
def generateLouvainCluster(edgeList):
    ...
    # For some cases no graph inferred
    if len(graph.es.indices)==0:
        print('ERROR! No graph found, please recheck the parameters!')
        exit(0)
...
def generateLeidenCluster(edgeList):
    """
...
def preprocessSpatial(originalMatrix):
    ...
    # D = rangex if rangex >= rangey else rangey
    # D = 1
    return spatialMatrix
