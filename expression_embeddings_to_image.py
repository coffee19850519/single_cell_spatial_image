...
def transform_expression_embedding_to_RGB(expression_file):
    ...
    embedding_values = X.iloc[:, 9:].values
    ...
    for percentile_0 in range(0, 50):
        for percentile_1 in range(0, 50):
            for percentile_2 in range(0, 50):
                ...
                temp_X_transform = np.zeros_like(embedding_values, np.int8)
                temp_X_transform[:, 0] = scale_to_RGB(embedding_values[:, 0], percentile_0)
                temp_X_transform[:, 1] = scale_to_RGB(embedding_values[:, 1], percentile_1)
                temp_X_transform[:, 2] = scale_to_RGB(embedding_values[:, 2], percentile_2)

                pcc, perm = correlation_between_images(X.iloc[:, 6:9].values, embedding_values)
                if best_pcc < pcc:
                    best_pcc = pcc
                    best_perm = perm
    ...
    embedding_values[:, 0] = scale_to_RGB(embedding_values[:, 0], best_percentile_0)
    embedding_values[:, 1] = scale_to_RGB(embedding_values[:, 1], best_percentile_1)
    embedding_values[:, 2] = scale_to_RGB(embedding_values[:, 2], best_percentile_2)

    print('the best pcc for the file {:s} is {:.3f}'.format(expression_file,best_pcc))

    save_transformed_RGB_to_image_and_csv(X, embedding_values[:, best_perm], expression_file)



    del embedding_values, data, X
...
data_folder = r'/run/media/fei/Entertainment/phate_embeddings/spatial_vel/1.0.0/'
for expression_file in os.listdir(data_folder):
    if os.path.splitext(expression_file)[1] == '.csv':
        transform_expression_embedding_to_RGB(os.path.join(data_folder, expression_file))
